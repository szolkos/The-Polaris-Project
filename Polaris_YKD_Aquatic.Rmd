...
```{r DATA EXPLORATION, echo=FALSE}
  ...
  # ANOVA: test for differences between...
    ## ... fen environments/ponds
      ...
      fit1 <- aov((dexcess) ~ FenDetail, ykd_sub)
      TukeyHSD(fit1)
    ## ... burn year*fen environments/ponds
      unique(ykd_sub$LandscapeCategory)
      fit2 <- aov((dexcess) ~ BurnYear*LandscapeCategory, ykd_sub)
      TukeyHSD(fit2)
    ## ... fen environments/ponds*year
      unique(ykd_sub$LandscapeCategory)
      fit3 <- aov((dexcess) ~ LandscapeCategory*Year, ykd_sub)
      TukeyHSD(fit3)
  ...
  # Explore data: simple graphs
    ## Boxplot
    ggplot(ykd_sub, aes(y=(dexcess), x=factor(LandscapeCategory), fill=BurnYear)) + # FenDetail, BurnYear, Year
      #geom_boxplot() +
      geom_boxplot(aes(fill=factor(BurnYear))) +
      theme_bw() +
